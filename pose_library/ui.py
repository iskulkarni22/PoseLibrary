...
def show_ui():
    ...
    # save pose section
    cmds.setParent("..")
    cmds.frameLayout(label='Save New Pose',
                     collapsable=True,
...
def remove_selected_pose(*args):
    """ Remove selected pose from pose library. """
    selected_pose = get_selected_pose()
    if selected_pose is None:
        return

    ans = cmds.confirmDialog(title='Confirm',
                       message='Are you sure you want to remove pose "{}"?'.format(selected_pose),
                       button=['Yes', 'No'],
                       defaultButton='Yes',
                       cancelButton='No',
                       dismissString='No')
    ...
    res = core.remove_pose_from_library(pose_file)
    if res:
        cmds.confirmDialog(message='Successfully removed pose: {}'.format(selected_pose))
    else:
        cmds.confirmDialog(message='File does not exist for pose: {}'.format(selected_pose))
